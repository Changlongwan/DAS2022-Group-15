...
Therefore we should exclude the sellable_online in the future model.
...
model.depth %>%
               summary() %>%
               coef()%>%
               kable(caption = 'Estimates of the parameters from the model.depth') %>%
               kable_styling(latex_options = 'HOLD_position', )
...
# 95% confidence interval
confint(model.2) %>%
  kable(caption = '\\label{tab:cimult} Confidence interval of the point estimate in multivariate model.1') %>%
  kable_styling(font_size = 10, latex_options = "hold_position")
```
...
modelA <- glance(glm(price ~ category, data = IKEA, family = binomial(link = "logit")))
modelB <- glance(glm(price ~ sellable_online, data = IKEA, family = binomial(link = "logit")))
modelC <- glance(glm(price ~ other_colors, data = IKEA, family = binomial(link = "logit")))
modelD <- glance(glm(price ~ depth, data = IKEA, family = binomial(link = "logit")))
modelE <- glance(glm(price ~ height, data = IKEA, family = binomial(link = "logit")))
modelF <- glance(glm(price ~ width, data = IKEA, family = binomial(link = "logit")))
modelG <- glance(glm(price ~ other_colors + depth + height + width, data = IKEA, family = binomial(link = "logit")))
Models <- c('SGLM(category)','SGLM(sellable_online)','SGLM(other_colors)','SGLM(depth)','SGLM(height)','SGLM(width)','MGLM(four variables)')
bind_rows(modelA,modelB,modelC,modelD,modelE, modelF,modelG,.id="Model") %>%
...
The Multivariate Generalized Linear Models has the highest R2 adj value and the lowest AIC & BIC values. This result is the same as the inference using confidence intervals. We should contain other_colors, depth, height and width in the model.
